...
serve(async (req) => {
    ...
    try {
        ...
        // Using Africa/Addis_Ababa (UTC+3) as per existing code
        const now = new Date();
        ...
        if (bookingConfig && bookingConfig.is_active) {
            if (bookingConfig.show_booking_service) {
                if (businessType === 'hotel') {
                    ...
                    serviceInstructions.push(forcedLanguage === 'somali' ? '💼 Taageerada macaamiisha iyo su\'aalaha' : '💼 Customer support and answering questions');
